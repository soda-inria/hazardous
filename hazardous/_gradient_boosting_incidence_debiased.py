...
from ._ipcw import IPCWLoopEstimator
from .metrics._brier_score import IncidenceScoreComputer
from .utils import check_y_survival


class WeightedBinaryTargetEstimator(IncidenceScoreComputer):
    ...
    def draw(self, X, ipcw_training=False):
        ...
        if ipcw_training:
            # The event corresponds to the censoring.
        ...
        if ipcw_training:
            ...
            y = y_binary * self.event_train

        return times.reshape(-1, 1), y, sample_weight

    def fit(self, X):
        for _ in range(self.n_iter_before_feedback):
            sampled_times, y_binary, sample_weight = self.draw(X, ipcw_training=True)
        ...
        # Update 'ipcw_train', which is the IPCW evaluated at the training durations.
...
class GradientBoostingIncidenceDebiased(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, times=None):
        ...
        ipcw_est = IPCWLoopEstimator(incidence_est=self.estimator_)
        self.weighted_targets_ = WeightedBinaryTargetEstimator(
            y,
        ...
        iterator = range(1, self.n_iter + 1)
        ...
        for idx_iter in iterator:
            ...
            self.estimator_.fit(X_with_time, y_multiclass, sample_weight=sample_weight)
        ...
        if self.event_of_interest == "any":
            event_classes = [f"event_{idx}" for idx in range(1, len(self.event_ids_))]
            self.classes_ = np.array(["no_event", *event_classes])
        else:
    ...
    def predict_cumulative_incidence(self, X, times=None):
        """Predict the cumulative incidence function (CIF).
        ...
        times : ndarray of shape (n_times)
            The time steps to make prediction at, for each sample and
            for each unique event.

        Returns
        -------
        predicted_curved : ndarray of shape (n_samples, n_times, len(classes_))
        """
        if times is None:
        ...
        n_samples, n_times, n_events = X.shape[0], len(times), len(self.classes_)
        predicted_curves = np.reshape(
            predictions_at_all_times, (n_times, n_samples, n_events)
        ).swapaxes(0, 1)
